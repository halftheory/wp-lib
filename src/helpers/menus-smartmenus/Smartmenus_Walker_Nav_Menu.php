<?php
namespace Halftheory\Lib\helpers\Menus_Smartmenus;
...
class Smartmenus_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		if ( ! empty($classes) ) {
			...
			foreach ( $classes as $key => $value ) {
				if ( str_replace($keep, '', $value) === $value ) {
					unset($classes[ $key ]);
				}
			}
		}
		$classes[] = (int) $depth === 0 ? 'sm-nav-item' : 'sm-sub-item';
		 ...
		 */
		$atts['class'] = array();
		$atts['class'][] = (int) $depth === 0 ? 'sm-nav-link' : 'sm-sub-link';
		if ( is_object($menu_item) && isset($menu_item->classes) && is_array($menu_item->classes) ) {
			if ( in_array('menu-item-has-children', $menu_item->classes) ) {
				$atts['class'][] = 'sm-sub-toggler';
			}
		}
		$atts['class'] = implode(' ', array_unique($atts['class']));
